...
class AvgNet(torch.nn.Module):
    def __init__(self, num_classes:int):
        super(AvgNet, self).__init__()

        self.num_filter = 1  # self.num_filter决定了对角线的条数
        max_nodes = 128
        # 用于得到对角线相关数据
    ...
    def forward(self, x_in, mask=None):  # [batch_size, 1, length, 2]
        x_in = torch.unsqueeze(x_in, dim=1).permute(0, 1, 3, 2)
        # 处理I通道数据。得到邻接矩阵（其实是特征矩阵）
        adj_i = torch.zeros(x_in.shape[0], x_in.shape[2], x_in.shape[2]).to(x_in.device)  # (batch_size, length, length)
        ...
        adj_q = torch.zeros(x_in.shape[0], x_in.shape[2], x_in.shape[2]).to(x_in.device)  # (batch_size, length, length)
        x_in_q = x_in[:, :, :, 1]  # [batch_size, 1, length]
