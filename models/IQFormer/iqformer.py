...
class IQFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        stft = torch.squeeze(self.patch_embedSTFT(stft), dim=2)
...
def iq_former_128(num_classes:int, sig_size:int):
...
def iq_former_1024(num_classes:int, sig_size:int):
...
if __name__ == '__main__':
    input_x = torch.rand(1, 2, 128)
    input_stft = torch.rand(1, 1, 32, 128)
    input_data = (input_x, input_stft)
    model = iq_former_128(11, 128)
    test_model_performance(model, input_data)

    # input_x = torch.rand(1, 2, 1024)
    # input_stft = torch.rand(1, 1, 32, 1024)
    # input_data = (input_x, input_stft)
    # model = iq_former_1024(24, 1024)
    # test_model_performance(model, input_data)